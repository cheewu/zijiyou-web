...
function tpl_article_search($query_word, $region_name, $pg, $ps = 5) {
    ...
    foreach ($documents AS &$document) {
    	...
    	$document['fragementID'] = strval($document['_id']);
    	...
    	foreach ($selected_paragraphs AS &$paragraph) {
    	    $paragraph = preg_replace("#(<\s*img.*?/\s*>)#", "", $paragraph);
    	    $paragraph = strip_tags($paragraph);
    	    $paragraph = tpl_trim($paragraph);
    	    $paragraph = h($paragraph);
    	}
    	unset($paragraph);
    	$document['content'] = array_filter($selected_paragraphs, 'tpl_trim');
...
function _tpl_article_summary($selected_paragraphs, $max_count, $callback)
{
    $count = 0;
    $max_paragraph_count = 5;
    $output_paragraph = array();
    foreach ($selected_paragraphs AS $index => $paragraph) {
        $length = mb_strlen($paragraph, "utf-8");
        $sub_paragraph = "";
        for ($i = 0; $i < $length; $i++) {
            $sub_char = mb_substr($paragraph, $i, 1, "utf-8");
            strlen($sub_char) <= 1 ? $count ++ : $count += 2;
            if ($count < $max_count) {
                $sub_paragraph .= $sub_char;
            } else {
                break;
            }
        }
        if (count($output_paragraph) < $max_paragraph_count) {
            $output_paragraph[] = $sub_paragraph;
        } else {
            break;
        }
        if ($count > $max_count) { break; }
    }
    return implode("", array_map($callback, $output_paragraph));
}

function tpl_article_summary($selected_paragraphs, $has_image) {
    $max_count = $has_image ? 300 : 600;
    return _tpl_article_summary($selected_paragraphs, $max_count, 
            function ($v) { 
              return "<p class='paragraph-summary'>$v</p>"; });
}

function tpl_mobile_article_summary($selected_paragraphs, $has_image) {
  $max_count = $has_image ? 75 : 150;
  return _tpl_article_summary($selected_paragraphs, $max_count, 
            function ($v) { 
              return "<p class='font-li-ct'>$v</p>"; });
}

function tpl_mobile_article_has_image($images) {
    if (!is_array($images)) { return false; }
    list($images, $width, $height) = tpl_article_sort_filter($images, 'width', 130, 120);
    if (count($images) == 0) { return false; }
    return get_article_image($images[0], 130, 0); 
}

function tpl_echo_article_image($images) {
    if (!is_array($images)) { return; }
    list($images, $width, $height) = tpl_article_sort_filter($images, 'width', 205, 110);
    if (($case = $count = count($images)) == 0) { return false; } 
    if ($count < 3 && $width < 640) { return false; }

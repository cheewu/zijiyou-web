...
$sub_destination = @$correlation['correlation']['destination'] ?: array();
$sub_region = array();
foreach($sub_destination AS $region_name => $score) {
    if (count($sub_region) >= 5) { break; }
    $region_detail = $_SGLOBAL['db']->keywordMap_select_one(array('name' => $region_name));
    if (empty($region_detail) || empty($region_detail['regionID'])) { continue; }
    $sub_region[] = array('name' => $region_name, '_id' => $region_detail['regionID']); 
}

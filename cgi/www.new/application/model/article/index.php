...
if ($_GET['mobile']) {
  ...
  if ($_GET['type'] == 'poi') {
    $poi    = $_SGLOBAL['db']->POI_select_one(array('_id' => new MongoId($_GET['id'])));
    $region = $_SGLOBAL['db']->Region_select_one(array('_id' => $poi['regionId']));
    $keyword = $poi['name'];
    $name    = $region['name'];
  } else {
    $region = $_SGLOBAL['db']->Region_select_one(array('_id' => new MongoId($_GET['id'])));
    $keyword = $name = $region['name'];
  }
  list($documents, $total_res_cnt) = tpl_article_search($keyword, $name, $pg, $ps);
  if (empty($_GET['page_down'])) {
  ...
  foreach ($documents AS $article) {
    ...
    $res[] = array(
      'href'     => "/detail/$r_id/$a_id?mobile=1",
      'image'    => @$image ?: '',
      'title'    => @$article['title'] ?: $region['name']."游记",
      'shortcut' => tpl_mobile_article_summary($article['content'], $image)
    );
  }

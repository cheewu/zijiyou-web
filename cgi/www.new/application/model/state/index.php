...
foreach($sub_region AS $index => $value){
	if(count($sub_region_geo) >= 10) { break; }
	$id = (string)$value['_id'];
	//筛选有坐标点的poi
...
$sub_attraction = @$region_correlation['correlation']['attraction'] ?: array();
$sub_poi = array();
foreach($sub_attraction AS $poi_name => $score) {
    if (count($sub_poi) >= 10) { break; }
    $poi_detail = $_SGLOBAL['db']->keywordMap_select_one(array('name' => $poi_name));
    if (empty($poi_detail) || empty($poi_detail['poiID'])) { continue; }
    $sub_poi[] = array('name' => $poi_name, '_id' => current($poi_detail['poiID'])); 
}

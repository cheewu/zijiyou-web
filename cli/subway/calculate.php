...
foreach ($poi_arr AS $index => $poi) {
  ...
  printf("\r(%s/%d) {$distance}km 内临近地铁站个数:%d POI.name={$poi['name']}%s\r", 
         str_pad($index + 1, strlen($poi_cnt), '0', STR_PAD_LEFT),
         $poi_cnt, count($nearby_subway_station), str_repeat(" ", 30));
  ...
  $tripfm->POI->update(array('_id' => $poi['_id']), 
                       array('$set' => array('nearSubwayStation' => $subway_field)));
}
echo PHP_EOL;
...
function mongo_geo_query(MongoDB $db, $collection, array $center, $radius = 0, 
  ...
  return !empty($res['results']) ? $res['results'] : array();
} 
